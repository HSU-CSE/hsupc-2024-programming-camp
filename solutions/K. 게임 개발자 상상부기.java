...
public class Main {
    static class ManageRoom{
        ...
        public boolean gameStart(String title,int id){//어떤 아이가 게임을 시작했는지 중요함
            boolean ret = false;
            for(Room r:data){
    ...
    public static void main(String[] args)throws IOException{
        ...
        for(int i=0;i<n;i++){//n번
            ...
            switch (mode) {
                ...
                case "start"->{
                    String title = log.nextToken();
                    boolean flag = mr.gameStart(title, id);
                    if(!flag){//id 안 맞는 아이가 시작함.
                        errorAdd(errors, new Error(time,id), Numbering);
                    }
                }  
                case "end"->{
        ...
        for(Error output:errors){
            sb.append(output.getTime() + " " +output.getId()+"\n");
        }
