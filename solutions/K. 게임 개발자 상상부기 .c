...
void setAdminId(Room* it, int id) {
	it->adminId = id;
}
bool enterRoom(Manage* it, int idx, int id) {
...
bool GameStart(Manage* it, char title[], int id) {
	...
	if (it->data[idx].adminId == id)return true;
	return false;
}
void removeValue(Manage* its, int idx) {
	...
	its->lastidx--;//마지막 인자 하나 줄이기
}
bool GameEnd(Manage* it, char title[], int id) {
	bool ret = false;
	int idx = AlreadyMake(it, title);
	if (it->data[idx].adminId == id) ret = true;
